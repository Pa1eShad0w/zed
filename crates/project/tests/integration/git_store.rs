...
mod repository_activation_tests {
    ...
    #[gpui::test]
    async fn opening_read_only_file_in_parked_repository_settles_permission(
        cx: &mut TestAppContext,
    ) {
        init_test(cx);
        ...
        .await;
        fs.set_readonly(path!("/root/a/repo/x.txt"), true);
        assert_repositories(&project, cx, &[], &[path!("/root/a/repo")]);

        let buffer = project
            .update(cx, |project, cx| {
                project.open_local_buffer(path!("/root/a/repo/x.txt"), cx)
            })
            .await
            .unwrap();
        cx.executor().run_until_parked();
        ...
        project.read_with(cx, |project, cx| {
            assert!(
                project
                    ...
                    .is_empty(),
                "activating the repository must settle the buffer's pending permission check"
            );
            assert_eq!(buffer.read(cx).capability(), language::Capability::Read);
        });
    }

...
use language::{
    Anchor, Buffer, BufferEvent, Capability, DiskState, Language, LanguageRegistry, decode_text,
    encode_text,
...
struct GitDiffOperations {
    ...
    kind: DiffKind,
    /// Perforce has no staging index, so its diffs must never offer stage/unstage.
    has_staging_index: bool,
}

impl DiffOperations for GitDiffOperations {
    fn supports_staging(&self) -> bool {
        self.has_staging_index && matches!(self.kind, DiffKind::Unstaged | DiffKind::Uncommitted)
    }

    fn supports_unstaging(&self) -> bool {
        self.has_staging_index && matches!(self.kind, DiffKind::Staged | DiffKind::Uncommitted)
    }
...
impl GitStore {
    ...
    pub fn perforce_move(
        ...
        let Some((dst_repo, dst_repo_path)) = self.repository_and_path_for_project_path(dst, cx)
        else {
            return Task::ready(Ok(()));
        };
...
mod tests {
    ...
    #[test]
    fn blame_entry_round_trip_preserves_backend_revision_labels() {
        for revision_label in [None, Some("@12345".to_string())] {
            let original = proto::BlameEntry {
                sha: vec![1; 20],
                start_line: 2,
                end_line: 5,
                original_line_number: 7,
                filename: "src/example.rs".to_string(),
                author: Some("Example Author".to_string()),
                revision_label,
                ..Default::default()
            };
            let decoded = deserialize_blame_entry(original.clone()).unwrap();
            assert_eq!(decoded.revision_label, original.revision_label);
            assert_eq!(serialize_blame_entry(decoded), original);
        }
    }

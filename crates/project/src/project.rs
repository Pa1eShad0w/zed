...
impl Project {
    ...
    pub fn trash_entry(
        ...
        let trash = worktree.update(cx, |worktree, cx| worktree.trash_entry(entry_id, cx))?;
        Some(cx.spawn(async move |_, _| {
            let result = trash.await?;
    ...
    pub fn delete_entry(
        ...
        // Perforce auto-checkout: open the file for delete in the depot. Same rationale as
        // `trash_entry` above; permanent deletion also reconciles the depot via `p4 delete`.
        let perforce_delete = self.perforce_delete_task(worktree_id, entry_id, &worktree, cx);

        let delete = worktree.update(cx, |worktree, cx| worktree.delete_entry(entry_id, cx))?;
        Some(cx.spawn(async move |_, _| {

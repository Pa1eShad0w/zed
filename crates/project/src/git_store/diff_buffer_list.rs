...
pub struct DiffBufferList {
    ...
    /// When `Some`, `load_buffers` only includes these repo paths, skipping the
    /// base-content load for every other changed file. Used by the Perforce
    /// changelist-scoped diff, where loading a file's diff base means a remote
    /// `p4 print` per file — an unscoped diff of a large workspace would fetch
    /// every opened file over the network. `None` (all git code paths) keeps
    /// the unfiltered whole-repo behavior.
    path_scope: Option<HashSet<RepoPath>>,
...
impl DiffBufferList {
    ...
    pub fn path_scope(&self) -> Option<&HashSet<RepoPath>> {
        self.path_scope.as_ref()
    }

    /// Restrict (or un-restrict, with `None`) the file list to the given repo
    /// paths. Emits `FileListChanged` so observers reload; a no-op when the
    /// scope is unchanged.
    pub fn set_path_scope(&mut self, scope: Option<HashSet<RepoPath>>, cx: &mut Context<Self>) {
    ...
    pub fn load_buffers(&mut self, cx: &mut Context<Self>) -> Vec<DiffBuffer> {
        ...
        self.project.update(cx, |_project, cx| {
            ...
            for item in repo.read(cx).cached_status() {
                ...
                // Scoped diff: skip out-of-scope files *before* `load_buffer`
                // spawns their base-load task, so the (possibly remote) base
                // fetch never runs for them.
                if let Some(scope) = &self.path_scope
                    && !scope.contains(&item.repo_path)
                {
                    continue;
                }

...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let perforce_panel =
            git_ui::perforce_panel::PerforcePanel::load(workspace_handle.clone(), cx.clone());
        let channels_panel =
        ...
        futures::join!(
            ...
            add_panel_when_ready(perforce_panel, workspace_handle.clone(), cx.clone()),
...
fn open_about_window(cx: &mut App) {
    fn about_window_icon(release_channel: ReleaseChannel) -> Arc<Image> {
        let bytes = match release_channel {
            ...
            // TODO(Phase ?): Fork-specific About-window icon. Reusing Stable icon for now.
            ReleaseChannel::Fork => include_bytes!("../resources/app-icon.png").as_slice(),
        };
    ...
    impl AboutWindow {
        fn new(cx: &mut Context<Self>) -> Self {
            ...
            let message: SharedString =
                format!("{release_channel_name} {version} {debug} — Perforce fork by @Pa1eShad0w")
                    .into();

...
| `a\\|\\|b` | empty middle alternative |";
    ...
    #[gpui::test]
    fn test_inline_code_chip_does_not_insert_whitespace(cx: &mut TestAppContext) {
        let style = MarkdownStyle {
            inline_code: TextStyleRefinement {
        ...
        assert_eq!(line_text.as_str(), "a b c");
        assert_eq!(rendered.lines.first().unwrap().code_chips.len(), 1);
    }

    #[gpui::test]
    fn test_inline_code_chip_copy_midline(cx: &mut TestAppContext) {
        let style = MarkdownStyle {
            inline_code: TextStyleRefinement {
                background_color: Some(gpui::red()),
                ..Default::default()
    ...
    #[gpui::test]
    fn test_inline_code_chip_copy_end_of_line(cx: &mut TestAppContext) {
        let style = MarkdownStyle {
            inline_code: TextStyleRefinement {
                background_color: Some(gpui::red()),
                ..Default::default()
    ...
    #[gpui::test]
    fn test_inline_code_chip_at_line_start_maps_rendered_zero(cx: &mut TestAppContext) {
        // The chip is paint-only; a line beginning with code must keep its first
        // rendered index mapped so hit testing cannot underflow.
        let style = MarkdownStyle {
            inline_code: TextStyleRefinement {
                background_color: Some(gpui::red()),
                ..Default::default()
        ...
        // With no inserted padding, the first glyph is 'b', after the opening backtick.
        assert_eq!(line.source_index_for_rendered_index(0), 1);
    }

...
impl GitGraph {
    ...
    fn perforce_change_label(&self, sha: Oid, cx: &mut App) -> Option<SharedString> {
        ...
        let data = repo.update(cx, |repo, cx| {
                ...
                .child(
                    v_flex()
                            ...
                            div()
                                .on_mouse_down(
                                    ...
                                    cx.listener(|this, event: &MouseDownEvent, window, cx| {
                                        // The hide-column menu lists the git graph's columns;
                                        // they don't map onto the Perforce file-history table.
                                        if this.perforce_file_history {
                                            return;
                                        }
                                ...
                                .child(render_table_header(
                                    // Perforce file history prepends a Revision (`#<rev>`) column
                                    // and appends a Branch column to the shared file-history table.
                                    if self.perforce_file_history {
                                        TableRow::from_vec(
                                            vec![
                                                Label::new("Revision")
                        ...
                        .child({
                            ...
                            let commits_table =
                                Table::new(if self.perforce_file_history { 6 } else { 4 })
                                    ...
                                    .map_row(move |(index, row), window, cx| {
                                        ...
                                        let hover_bg =
                                            cx.theme().colors().element_hover.opacity(0.6);
                                        let selected_bg = if is_focused {
                                            cx.theme().colors().element_selected
                                        } else {
                                            cx.theme().colors().element_hover

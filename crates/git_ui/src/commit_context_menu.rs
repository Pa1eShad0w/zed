...
use git::{Oid, perforce::swarm_changelist_url};
...
pub(crate) fn commit_context_menu(
    ...
    // `None` for git or while the commit data is still loading — we then fall back to the sha.
    let p4_change: Option<SharedString> = repository
        .as_ref()
    ...
    // What the copy entry writes: the bare changelist number for Perforce, else the full sha.
    let copy_text: String = p4_change
        .as_ref()

...
pub struct ProjectDiff {
    ...
    /// When `Some`, this tab is a Perforce changelist-scoped diff: the
    /// underlying `DiffBufferList` carries a matching `path_scope`, the tab
    /// shows this title (e.g. `Changelist #12345` / `Default Changelist`),
...
impl ProjectDiff {
    ...
    pub fn deploy_at(
        ...
        let existing = workspace
            ...
            .find(|item| item.read(cx).scope_title.is_none());
    ...
    pub fn deploy_changelist(
        ...
        if let Some(existing) = existing {
            existing.update(cx, |project_diff, cx| {
                // The changelist's file set may have changed since last open; re-scoping
                // emits `FileListChanged`, which triggers a refresh (no-op if unchanged).
                let branch_diff = project_diff.diff.read(cx).branch_diff().clone();
                branch_diff.update(cx, |branch_diff, cx| {
                    branch_diff.set_path_scope(Some(paths), cx);
                });
            });
            workspace.activate_item(&existing, true, true, window, cx);
            return;
        ...
        // Scope + repo are set synchronously here: `new` spawns its first refresh as an
        // async task that only polls after the current update returns, so the scope is in
        // place before the first `load_buffers` call — no unscoped base loads ever start.
        project_diff.update(cx, |project_diff, cx| {
            let branch_diff = project_diff.diff.read(cx).branch_diff().clone();
            branch_diff.update(cx, |branch_diff, cx| {
                branch_diff.set_path_scope(Some(paths), cx);
            });
            project_diff.set_repo(Some(repo), cx);
        });
...
impl Item for ProjectDiff {
        ...
        Self: Sized,
    {
        ...
        // A changelist-scoped tab splits into an equally-scoped clone, not a
        // whole-repo diff (which would kick off base loads for every file).
        let scope_title = self.scope_title.clone();
        let path_scope = self
            .diff
            .read(cx)
            .branch_diff()
        ...
        Task::ready(Some(cx.new(|cx| {
            let mut project_diff = ProjectDiff::new(self.project.clone(), workspace, window, cx);
            project_diff.scope_title = scope_title;
            if path_scope.is_some() {
                let branch_diff = project_diff.diff.read(cx).branch_diff().clone();
                branch_diff.update(cx, |branch_diff, cx| {
                    branch_diff.set_path_scope(path_scope, cx);
                });
            }
            project_diff
        })))
...
impl SerializableItem for ProjectDiff {
    ...
    fn serialize(
        ...
        // rebuilds the plain whole-repo diff, which on Perforce would start a
        // base load (remote `p4 print`) for every opened file at startup.
        if self.scope_title.is_some() {
            return None;
        }
        Some(Task::ready(Ok(())))
    }

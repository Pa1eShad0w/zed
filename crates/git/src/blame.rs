...
impl BlameEntry {
    ...
    pub fn revision_target(&self, highlighted_sha: Option<Oid>) -> Option<(Oid, RepoPath)> {
        // Non-Git backends use synthetic OIDs that cannot be resolved as Git revisions.
        if self.revision_label.is_some() || self.sha.is_zero() || Some(self.sha) == highlighted_sha
        {
            return None;
        }
    ...
    pub fn previous_revision_target(&self) -> Option<(Oid, RepoPath)> {
        if self.revision_label.is_some() {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn non_git_blame_entries_do_not_offer_git_revision_targets() {
        let mut entry =
            BlameEntry::new_from_blame_line("1111111111111111111111111111111111111111 1 1 1")
                .unwrap();
        entry.filename = "file.txt".into();
        entry.previous = Some("2222222222222222222222222222222222222222 file.txt".into());
        assert!(entry.revision_target(None).is_some());
        assert!(entry.previous_revision_target().is_some());

        entry.revision_label = Some("@12345".into());
        assert!(entry.revision_target(None).is_none());
        assert!(entry.previous_revision_target().is_none());
    }

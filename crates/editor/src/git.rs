...
pub fn render_diff_hunk_controls(
    ...
    // Perforce reverts a hunk to the depot `#have` text rather than restoring from an
    // index, so the Restore button is relabeled "Revert" to match Perforce terminology.
        ...
        .when(show_stage_restore && supports_restore, |el| {
            el.child(
                ...
                .tooltip({
                    ...
                    move |_window, cx| {
                        Tooltip::for_action_in(tooltip_label, &::git::Restore, &focus_handle, cx)
                    }
                })
                .on_click({
                    let editor = editor.clone();
